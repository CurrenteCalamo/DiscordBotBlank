...
async function getPrivateRoom(oldState, newState) {
	...
	if (
		oldState.channel?.id !== data?.private_voices?.channelId &&
		oldState.channel?.parent?.id == data?.private_voices?.categoryId &&
	...
	if (data?.private_voices?.mode === true) {
		if (newState.channel?.id == channelId) {
			newState.guild.channels
				.create(`${newState.member.user.username}'s channel`, {
					type: 'GUILD_VOICE',
					parent: categoryId,
